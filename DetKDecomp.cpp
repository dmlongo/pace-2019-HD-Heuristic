	...
	if(nbr_sel_cov >= 0)
		do {
			...
			if(!add_edge || (MyK-nbr_sel_cov > 0)) {
				...
				} while(!tle && add_edge && (htree == nullptr) && (++i_add < add_edges.size()));
			}

		} while(!tle && (htree == nullptr) && ((nbr_sel_cov = setNextSubset(Connector, bound_edges, cov_sep_set, in_comp, cov_weights)) > 0));
