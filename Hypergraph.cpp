...
void Hypergraph::insertEdge(const HyperedgeSharedPtr &edge) {
	if (getEdgeByID(edge->getId()) != nullptr)
		writeErrorMsg(
				"This hypergraph already contains a Hyperedge with id "
						+ to_string(edge->getId()), "Hypergraph::insertEdge");
	...
	if (k <= Edges.size()) {
			...
			for (auto v : edges[indices[0]]->allVertices()) {
				...
				for (int i = 1; indices[i] != -1 && found; i++)
					if (find(edges[indices[i]]->allVertices().begin(),
							edges[indices[i]]->allVertices().end(), v)
		...
		for (auto v : edges[0]->allVertices()) {
			...
			for (int i = 1; i < edges.size() && found; i++)
				if (find(edges[i]->allVertices().begin(),
						edges[i]->allVertices().end(), v)
						== edges[i]->allVertices().end())
					found = false;
	...
	for (i = 1; i <= maxVC; i++) {
		...
		while ((indices = cit.next()) != nullptr && !shattered) {
			...
			for (powerset_type::iterator iter = pow_set_x.begin();
				...
				for (auto e_it = Edges.begin();
						e_it != Edges.end() && !edge_found; e_it++) {
					set_type help_x;

					for (auto v : set_x) {
						if (find((*e_it)->allVertices().begin(),
								(*e_it)->allVertices().end(), v)
